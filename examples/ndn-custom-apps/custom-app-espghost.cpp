...
ESPGhost::StartApplication()
{
  ...
  this->sensorRequestCounter = 0;

  this->sensorRequests = 250;
  int startSensorRequestsAt = 20;
  ...
  //Simulator::Schedule(Seconds(59.9),&ESPGhost::PrintBlockchain, this);
}
...
std::string 
ESPGhost::ESPCommunicate(int len, std::string prefix, std::string v, std::string n, std::string h, std::string i)
{
...
ESPGhost::OnData(std::shared_ptr<const ndn::Data> data)
{
  ...
  std::cout << "THE NAME OF THE DATA IS:" << sensor << " - " << this->sensorRequestCounter << "/" << this->sensorRequests << std::endl; 
  if(sensor == "sensordata")
  {
    ...
    if(pr != "e")
    {
      std::cout << "ESPGhost:OnData:sensordata: reply from ESP is : " << pr << std::endl;
      if(this->sensorRequestCounter < this->sensorRequests)
      {
