...
# data = subset(data, Type %in% c("OutInterests"))

# combine stats from all faces
data.combined = summaryBy(. ~ Time + Node + Type, data=data, FUN=sum)

data.root = subset (data.combined, Node == "ESP")
#data.leaves = subset(data.combined, Node %in% c("ConsumerOne", "ConsumerTwo", "ConsumerThree"))

# graph rates on all nodes in Kilobits
g.all <- ggplot(data.combined) +
  geom_point(aes (x=Time, y=Kilobits.sum, color=Type), size=1) +
  geom_line(aes (x=Time, y=Kilobits.sum, color=Type), size=0.1) +
  ylab("Rate [Kbits/s]") +
  facet_wrap(~ Node)# +
  #ylim(0,75)
