...
#include "custom-app-consumerone.hpp"
...
NS_LOG_COMPONENT_DEFINE("ConsumerOneApp");

using namespace BlockNDN;

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED(ConsumerOneApp);

// register NS-3 type
TypeId
ConsumerOneApp::GetTypeId()
{
  static TypeId tid = TypeId("ConsumerOneApp")
  .SetParent<ndn::App>()
  .AddConstructor<ConsumerOneApp>();
  return tid;
}

// Processing upon start of the application
void
ConsumerOneApp::StartApplication()
{
  this->myName = "ConsumerOneApp";
  ...
  //Simulator::Schedule(Seconds(39),&ConsumerOneApp::PrintLocalBlockchain, this);
  //Simulator::Schedule(Seconds(59),&ConsumerOneApp::PrintPrepareLog, this);
}


// Processing when application is stopped
void
ConsumerOneApp::StopApplication()
{
...
void
ConsumerOneApp::OnInterest(std::shared_ptr<const ndn::Interest> interest)
{
  ...
  //NS_LOG_DEBUG("[ConsumerOneApp]: Received Interest packet for " << prefix.substr(0,10));
...
bool ConsumerOneApp::checkPrepareConsensus(){
  int quorum = 0;
...
void
ConsumerOneApp::PrintLocalBlockchain()
{
  this->blockChainProxy.PrintBlockChainToScreen(this->myName);
};

void
ConsumerOneApp::PrintPrepareLog()
{
    std::cout << "********************************************************" << std::endl;
...
void ConsumerOneApp::clearLocalData()
{
  Block empty;
  BFT bft;
...
void
ConsumerOneApp::OnData(std::shared_ptr<const ndn::Data> data)
{
