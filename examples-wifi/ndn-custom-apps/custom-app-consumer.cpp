...
#include "custom-app-consumer.hpp"
...
ConsumerApp::StartApplication()
{
  ...
  //Simulator::Schedule(Seconds(59),&ConsumerApp::PrintLocalBlockchain, this);
  Simulator::Schedule(Seconds(59),&ConsumerApp::PrintPrepareLog, this);
}
...
ConsumerApp::OnInterest(std::shared_ptr<const ndn::Interest> interest)
{
  ...
  //NS_LOG_DEBUG("[ConsumerApp]: Received Interest packet for " << prefix.substr(0,10));
...
bool ConsumerApp::checkPrepareConsensus(){
  int quorum = 0;
...
void
ConsumerApp::PrintLocalBlockchain()
{
  this->blockChainProxy.PrintBlockChainToScreen(this->myName);
};

void
ConsumerApp::PrintPrepareLog()
{
    std::cout << "********************************************************" << std::endl;
...
void ConsumerApp::clearLocalData()
{
